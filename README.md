# Bhargav Rishi — Static Portfolio

Pure HTML/CSS/JS portfolio (no build tools). Open `index.html` in a browser to view.

## File structure
- `index.html` — markup, navigation, hero/profile/featured/all-projects sections, plus case study and hero CTAs.
- `style.css` — layout, theming, cards, responsive tweaks, button and chip styles.
- `main.js` — data-driven content (skills, strengths, projects, results, links) and DOM rendering.
- `assets/` — local images used by the header, logos, resume PDF, and project cards.

## Technologies
- HTML5 + vanilla CSS (custom theming, grids/flex, scroll offsets for anchors).
- Vanilla JS for rendering skills, strengths, and project cards from data.
- Local assets; some projects link to external GitHub repos for details.

## Editing content
- Update skills (core vs additional), strengths, projects (titles, summaries, results, tags, links, images), and contact email in `main.js`.
- Adjust layout/colors/typography, spacing, and button styles in `style.css`.
- Swap the header photo, education/experience logos, resume PDF, and project thumbnails in `assets/`.

## Deploying to GitHub Pages
1. Create a repo named `USERNAME.github.io` on GitHub.
2. Copy `index.html`, `style.css`, `main.js`, and the `assets/` folder into the repo root.
3. Enable Pages (Settings → Pages → Source: Deploy from branch). It will serve at `https://USERNAME.github.io`.

No build step required.
